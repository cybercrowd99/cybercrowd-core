/**
 * CyberCrowd CORE — Function Boundary
 *
 * Lane: core/
 *
 * Owns:
 * - safe local CyberCrowd Core function entry
 *
 * Does Not Own:
 * - UI
 * - NET
 ...
 *
 * Receives:
 * - local function request for found/result/object signal
 *
 * Sends To:
 * - CyberCrowdCoreReady
 * - CyberCrowdCoreFlow
...
const CyberCrowdCoreFunction = (() => {
  ...
  function normalizeRequest(input = {}) {
    return {
      ...
      expected_home: cleanText(
        input.expected_home ||
        input.target_home ||
        ""
      ),

      expected_lane: cleanText(
        input.expected_lane ||
        input.target_lane ||
        ""
      ),

      lost_recipient: cleanText(
  ...
  function notReady(reason, extra = {}) {
    return {
      ...
      missing: Array.isArray(extra.missing)
        ? extra.missing
        : [],
  ...
  function ready() {
    ...
    if (!readyCheck || typeof readyCheck.run !== "function") {
      return notReady(
        "CyberCrowdCoreReady is not loaded"
      );
    }
  ...
  function found(input = {}) {
    ...
    if (!flow || typeof flow.runFoundFlow !== "function") {
      return notReady(
        "CyberCrowdCoreFlow is not loaded"
      );
    }
  ...
  function explain(result) {
    ...
    if (result && result.status === "not_ready") {
      return (
        "CyberCrowd Core function is not ready. Missing: " +
        (result.missing || []).join(", ")
      );
    }
...
export default CyberCrowdCoreFunction;
